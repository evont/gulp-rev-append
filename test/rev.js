...
test('adds the revision hash before the first `.` in the filename', async t => {
	...
	const file = await data;
	t.is(file.path, 'unicorn.css.map');
...
test.cb('handles sourcemaps transparently', t => {
	...
	stream.on('data', file => {
		if (path.extname(file.path) === '.map') {
			t.is(file.path, path.normalize('maps/pastissada.css.map'));
			t.end();
		}
	});
...
test.cb('handles unparseable sourcemaps correctly', t => {
	...
	stream.on('data', file => {
		if (path.extname(file.path) === '.map') {
			t.is(file.path, 'pastissada.css.map');
			t.end();
		}
	});
...
test('handles a `.` in the folder name', async t => {
	...
	const file = await data;
	t.is(file.path, path.normalize('mysite.io/unicorn.css'));
